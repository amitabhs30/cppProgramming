...
#define numSetBits(x) __builtin_popcountll(x)
...
#define print2d(x)  \
    for(auto it1=x.begin();it1!=x.end();it1++) {print((*it1));}
#define printii(x)  \
        for(auto it=x.begin();it!=x.end();it++) cout<<it->F<<' '<<it->S<<endl;  
...
signed main()
{
    ...
    fore(t,T)   
    {
        ...
        <<(double)(clock()-clk)/CLOCKS_PER_SEC<<endl;
#endif
}
